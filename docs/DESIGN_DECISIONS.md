# Vör — Design Decisions (archived)

> Verbatim copy of the root `README.md` as it stood on 2026-08-25, before that
> file was reworked into a conventional project README.
>
> It's preserved because it is the only written record of *why* several
> choices were made — the asymmetric reconciliation, the two-part graduation
> gate, targeted evidence invalidation over a confidence float, Vertex AI over
> an API key — along with the hackathon track-selection rationale. That
> reasoning is worth keeping even though it doesn't belong in a README.
>
> **Treat this as a historical record, not current documentation.** Parts are
> already out of date: the blast-radius table has since moved to Firestore,
> the default model is now set via `GEMINI_MODEL`, and `/classify` does have a
> trigger source (a Pub/Sub push subscription). For current behavior see the
> root `README.md` and `docs/DEPLOY.md`.
